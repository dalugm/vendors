...
TR_PATH="/root/terraria/${TR_VERSION}/Linux"

# 变量赋值：这一段是 grep 正则表达式和 awk 从文本中过滤出服务端进程的
# screen 进程号
PID=$(screen -ls | grep -E '[0-9]+\.[[:alnum:]]+' | grep "\.${TR_PROCESS}\>" | awk -F'.' '{print $1}')

# 逻辑判断：如果服务端进程的 screen 进程号不为空，也就是服务端处于启动状态
# 的时候（没在启动状态也就没有必要执行停止服务端操作了）
if [ -n "$PID" ]; then
    # 发送重启提醒
    screen -dr ${TR_PROCESS} -X stuff "say 服务器将在 20 秒后例行重启，请马上下线！\n"
    sleep 10
    screen -dr ${TR_PROCESS} -X stuff "say 服务器将在 10 秒后例行重启，请马上下线！\n"
    sleep 9
    screen -dr ${TR_PROCESS} -X stuff "say 服务器重启中！\n"
    sleep 1
    # 执行 exit 停止服务器
    screen -dr ${TR_PROCESS} -X stuff "exit\n"
    # 计数器，给下面的 while 循环使用
    TIMER=1
    ...
    while true; do
        ...
        if [ -z "$PID" ]; then
            cd ${TR_PATH} || return
            tar -zcvfP ~/backups/trworld_"$(date +%Y-%m-%d)".tar.gz ~/.local/share/Terraria/Worlds/1432-lizhong.wld
            bash start.sh
            break
        else
            ...
            # kill -9 强制杀掉服务端的 .net 进程。注意，服务端的 .net 进
            # 程号和其 screen 进程号之间的关系是：.net 进程号比 screen
            # 进程号大 1
            if [ $TIMER -gt 20 ]; then
                PID_PLUS=$(( PID+1 ))
                kill -9 "$PID_PLUS"
                sleep 3
                # 一般情况下 screen 中运行的服务端 .net 进程被结束后相应
